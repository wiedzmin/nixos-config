  ...
  config = mkMerge [
    (mkIf cfg.enable {
      ...
      home-manager.users."${user}" = {
        ...
        xdg.configFile = {
          "awesome/rc.lua".text = builtins.readFile ./rc.lua;
          "awesome/lib/controls.lua".text = readSubstituted config inputs pkgs [ ./subst/controls.nix ] [ ./lib/controls.lua ];
          "awesome/lib/menus.lua".text = readSubstituted config inputs pkgs [ ./subst/menus.nix ] [ ./lib/menus.lua ];
          "awesome/lib/themes.lua".text = readSubstituted config inputs pkgs [ ./subst/themes.nix ] [ ./lib/themes.lua ];
          "awesome/lib/utils.lua".text = builtins.readFile ./lib/utils.lua;
          "awesome/lib/widgets.lua".text = readSubstituted config inputs pkgs [ ./subst/widgets.nix ] [ ./lib/widgets.lua ];
          "awesome/lib/windows.lua".text = readSubstituted config inputs pkgs [ ./subst/windows.nix ] [ ./lib/windows.lua ];
          "awesome/themes" = { source = ./themes; recursive = true; };
          "awesome/lib/debug.lua".text = readSubstituted config inputs pkgs [ ./subst/debug.nix ] [ ./lib/debug.lua ];
        };
      };
    })
    (mkIf (cfg.enable && cfg.isDefault) {
      assertions = [{
        assertion = !config.wm.i3.isDefault && !config.wm.qtile.isDefault && !config.wm.herbstluft.isDefault;
