...
from pystdlib.uishim import get_selection, get_selection_rofi
...
if not (args.default_browser and args.fallback_browser):
    notify(f"[Search prompt]", f"Browsers not set, exiting...", urgency=URGENCY_CRITICAL, timeout=5000)
    sys.exit(1)
...
if searchengine:
    ...
    search_term = get_selection([], f"{searchengine} | term", font=args.selector_font).replace(" ", "+")
    if search_term:
