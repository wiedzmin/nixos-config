...
let
  cfg = config.ext.services.openvpn;
  makeOpenVPNJob =
    cfg: name:
    let
      ...
      configFile = pkgs.writeText "openvpn-config-${name}" ''
        ...
        ${optionalString (cfg.authUserPass != null && cfg.kpxc.dbpath == "") (
        ...
        ${optionalString (cfg.kpxc.dbpath != "") "auth-user-pass /run/secrets/vpn/${name}/auth_user_pass"}
      '';
    in
    {
      ...
      serviceConfig = {
        RuntimeDirectory = "secrets/vpn/${name}";
        RuntimeDirectoryMode = "755";
        ExecStartPre = optionalString (cfg.kpxc.dbpath != "")
          "/run/current-system/sw/bin/sh -c 'echo \"${cfg.kpxc.passphrase}\" | ${
            config.ext.security.keepassxc.package
          }/bin/keepassxc-cli show ${
            cfg.kpxc.dbpath
          } \"${cfg.kpxc.entry}\" -a username -sa password -q > /run/secrets/vpn/${name}/auth_user_pass'";
        ExecStart = "@${config.services.openvpn.package}/sbin/openvpn openvpn --suppress-timestamps --config ${configFile}";
        Restart = "always";
        Type = "notify";
      };
    };
        ...
        type =
          ...
          attrsOf (submodule {
            options = {
              ...
              kpxc.passphrase = mkOption {
                type = types.str;
                default = "";
                description = "Token for unlocking KeePassXC database";
              };
              kpxc.dbpath = mkOption {
                type = types.str;
                default = "";
              ...
              kpxc.entry = mkOption {
                type = types.str;
                default = "";
                description = "Name of entry to retrieve from KeePassXC database";
              };
