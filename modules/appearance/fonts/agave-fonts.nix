...
let
  cfg = config.appearance.fonts.agave;
  ...
  fontBeautified = baseFont // { family = "Agave ${if beautify then "Nerd Font" else ""}"; };
...
{
  options = {
    appearance.fonts.agave = {
      enable = mkOption {
        type = types.bool;
        default = false;
        description = "Whether to enable Agave font";
      };
    };
  };
