...
let
  cfg = config.appearance.fonts.victor;
  ...
  fontBeautified = baseFont // { family = "Victor Mono ${if beautify then "Nerd Font" else ""}"; };
...
{
  options = {
    appearance.fonts.victor = {
      enable = mkOption {
        type = types.bool;
        default = false;
        description = "Whether to enable Victor Mono font";
      };
    };
  };
  ...
  config = mkIf cfg.enable {
    fonts = {
      packages = with pkgs; [ victor-mono ];
