...
let
  cfg = config.appearance.fonts.geist;
  ...
  fontBeautified = baseFont // { family = "Geist Mono ${if beautify then "Nerd Font" else ""}"; };
...
{
  options = {
    appearance.fonts.geist = {
      enable = mkOption {
        type = types.bool;
        default = false;
        description = "Whether to enable Geist Mono font";
      };
    };
  };
