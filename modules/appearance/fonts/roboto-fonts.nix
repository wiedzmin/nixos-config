...
let
  cfg = config.appearance.fonts.roboto;
  ...
  fontBeautified = baseFont // { family = "Roboto Mono ${if beautify then "Nerd Font" else ""}"; };
  ...
  options = {
    appearance.fonts.roboto = {
      enable = mkOption {
        type = types.bool;
        default = false;
        description = "Whether to enable Roboto Mono font";
      };
    };
  };
