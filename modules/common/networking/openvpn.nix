...
let
  ...
  cfg = config.custom.networking.openvpn;
  ...
  makeOpenVPNJob = cfg: name:
    let
      ...
      configFile = pkgs.writeText "openvpn-config-${name}" ''
        ...
        ${cfg.config}
        ${optionalString (cfg.up != "" || cfg.updateResolvConf) "up ${pkgs.writeScript "openvpn-${name}-up" upScript}"}
...
in {
  imports = [ (mkRemovedOptionModule [ "custom" "networking" "openvpn" "enable" ] "") ];

  ###### interface

  options = {

    custom.networking.openvpn.servers = mkOption {
      default = { };
