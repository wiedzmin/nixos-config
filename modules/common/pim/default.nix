...
let
  ...
  tt_capture = writePythonScriptWithPythonPackages "tt_capture" [
    ...
    xprintidle_task = subprocess.Popen("${pkgs.xprintidle-ng}/bin/xprintidle-ng",
        env={
  ...
  emacsPimSetup = ''
    ...
    (use-package org
      ...
      (org-archive-location "${config.ide.emacs.orgDir}/journal.org::datetree/")
      (org-attach-directory "${config.ide.emacs.orgDir}/org-attach-data")
      ...
      (org-clock-x11idle-program-name "${pkgs.xprintidle-ng}/bin/xprintidle")
      ...
      (org-default-notes-file "${config.ide.emacs.orgDir}/refile.org")
      (org-ditaa-jar-path "${pkgs.ditaa}/lib/ditaa.jar")
      ...
      (deferred:$
        (deferred:nextc
          ...
          (lambda () (f-entries "${config.ide.emacs.orgDir}"
        ...
        (deferred:nextc
          ...
          (lambda () (f-entries "${config.secrets.job.workspaceRoot}"
                                (lambda (entry) (when (and (f-file? entry)
                                                           (s-suffix? "org" entry)
                                                           (not (s-contains? "journal" entry))
                                                           (file-exists-p entry))
...
in {
  ...
  config = mkMerge [
    ...
    (mkIf (cfg.enable && cfg.emacs.enable) {
      ...
      ide.emacs.config = ''${emacsPimSetup}'';
    })
  ];
}
