...
let
  ...
  collect_links_on_page = writePythonScriptWithPythonPackages "collect_links_on_page" [
    ...
    page_url_task = subprocess.Popen("${pkgs.xsel}/bin/xsel -o -b",
    ...
    if page_url is not None:
        if is_valid_url(page_url):
            ...
            with open("${cfg.sessions.firefox.path}/{0}.org".format(session_name), "w") as f:
  ...
  config = mkMerge [
    ...
    (mkIf (cfg.enable && cfg.firefox.enable && cfg.sessions.firefox.backup.enable) {
      ...
      wm.xmonad.keybindings = lib.optionalAttrs (config.wm.xmonad.enable) {
        ...
        "M-C-c" = ''spawn "${collect_links_on_page}/bin/collect_links_on_page"'';
      };
