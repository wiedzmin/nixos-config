  ...
  home-manager.users.kotya = {
    ...
    programs.zsh = {
      ...
      sessionVariables = {
        ...
        FZF_MARKS_FILE = "${config.users.extraUsers.kotya.home}/.bookmarks";
        FZF_MARKS_JUMP = "^[xjj";
        GREP_COLOR = "1;32";
        GREP_OPTIONS = "--color=auto";
        GTAGSLIBPATH = "${config.users.extraUsers.kotya.home}/.gtags/";
        HISTFILE = "${zshHistFilename}";
        PATH = "$PATH:${lib.concatStringsSep ":" (lib.unique binDirs)}";
        SHELL = "${pkgs.zsh}/bin/zsh";
        TMUXP_CONFIGDIR = "${config.users.extraUsers.kotya.home}/tmuxp";
        VISUAL = "${pkgs.emacs}/bin/emacsclient";
        WORKON_HOME = "${config.users.extraUsers.kotya.home}/.virtualenvs";
